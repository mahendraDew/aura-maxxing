...
export async function getNebiusDataGen (transcript: string) {
  try {
    console.log('Generating Nebius data concurrently...')

    const [revisionNotes, flashcards, projectList, quizzes, story] =
      await Promise.all([
          ...
          messages: [
            {
              ...
              content: `AI assistant is a brand new, powerful, human-like artificial intelligence.
                          ...
                          For response format, it should strictly follow this schema: ${QuizzesSchema}, each 
    ...
    // return { revisionNotes, flashcards, projectList, quizzes, story }
    const genText = {
    ...
    console.error('Error generating Nebius data:', error)
    throw error
  }
}
