...
export async function getGeminiData (transcript: string) {
  try {
    ...
    const revisionNotes_promise = generateText({
    ...
    const flashcard_promise= generateObject({
      model: google('gemini-2.5-flash'),
      schema: flashcardsSchema,
      prompt: `
        You are an AI assistant and an expert in programming education. 
    ...
    const quiz_promise = generateObject({
      model: google('gemini-2.5-flash'),
    ...
    const projectList_promise = generateObject({
      model: google('gemini-2.5-flash'),
    ...
    const story_promise = generateObject({
      model: google('gemini-2.5-flash'),
      schema: StorySchema,
      prompt: `
      AI assistant is a brand new, powerful, human-like artificial intelligence.
    ...
    const revisionNotes =
      results[0].status === 'fulfilled' ? results[0].value.text : ''

    const flashcards =
      results[1].status === 'fulfilled' ? results[1].value.object : []

    const quizzes =
      results[2].status === 'fulfilled' ? results[2].value.object : []

    const projectList =
      results[3].status === 'fulfilled' ? results[3].value.object : []

    const story =
      results[4].status === 'fulfilled'
        ? results[4].value.object
        : { title: '', paragraphs: [] }
