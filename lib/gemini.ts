...
export async function getGeminiData (transcript: string) {
  try {
    ...
    //       Flashcard 1:
    //         Front: What is Nodemailer?
    //         Back: A Node.js package for sending emails.
    ...
    // console.log(" gemini flashcard :", flashcards)
    ...
    const { object: quizzes } = await generateObject({
      model: google('gemini-1.5-flash'),
    ...
    //   temperature: 0.7
    // })
    const { object: projectList } = await generateObject({
      model: google('gemini-1.5-flash'),
